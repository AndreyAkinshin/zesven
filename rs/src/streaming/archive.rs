...
impl<R: Read + Seek + Send> StreamingArchive<R> {
    ...
    pub fn extract_entry_to<W: std::io::Write>(&mut self, path: &str, sink: &mut W) -> Result<u64> {
        ...
        // The locator is keyed by the entry's index in the parsed header
        // (`Entry::index`), not by its position in `self.entries` — the two
        // diverge once any entry was skipped for an invalid path.
        ...
        while let Some(entry_result) = iter.next() {
            match entry_result {
                Ok(entry) => {
                    ...
                    //
                    // Anti-items are materialized as empty files for parity
                    // with the canonical reader (`read::extraction`);
                    // parallel extraction skips them instead.
...
mod tests {
    ...
    fn test_build_entries_advances_streams_for_rejected_paths() {
        ...
        let (entries, skipped) =
            StreamingArchive::<std::io::Cursor<Vec<u8>>>::build_entries(&header);
